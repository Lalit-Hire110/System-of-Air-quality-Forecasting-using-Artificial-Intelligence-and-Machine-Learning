...
Beautiful nested progress bars in terminal with per-month and overall counts!
"""
...
class ColorfulFormatter(logging.Formatter):
    ...
    FORMATS = {
        logging.DEBUG: Fore.CYAN + format_template + Style.RESET_ALL,
        logging.INFO: Fore.GREEN + format_template + Style.RESET_ALL,
        logging.WARNING: Fore.YELLOW + format_template + Style.RESET_ALL,
        logging.ERROR: Fore.RED + format_template + Style.RESET_ALL,
        logging.CRITICAL: Back.RED + Fore.WHITE + format_template + Style.RESET_ALL
    }
...
file_handler = logging.FileHandler('logs/era5.log', encoding='utf-8')
file_handler.setLevel(logging.INFO)
file_formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')
file_handler.setFormatter(file_formatter)
console_handler = logging.StreamHandler(sys.stdout)  # Direct logging output to stdout
...
def load_config():
    ...
    tqdm.write(f"Loaded {len(variables)} variables and {len(stations)} stations from config.")
...
def month_temp_file_candidates(station_id, year, month):
    ...
    candidates.append(str(base / f"temp_{station_id}_{year}_{month}.nc"))
    candidates.append(str(base / f"temp_{station_id}_{year}_{month:02d}.nc"))
    candidates.append(str(base / f"{station_id}_{year}_{month:02d}.nc"))
    candidates.append(str(base / f"temp_{station_id}_{year}_{month}.zip"))
...
def fetch_variables_individually(client, station, year, month, variables):
    ...
    for v in variables.values():
        try:
            ...
            tqdm.write(f"Fetching variable {v} individually for {station_id} {year}-{month:02d}")
            ...
            if not is_valid_nc(tmp):
                tqdm.write(f"Warning: Individual fetch produced invalid file {tmp}")
            ...
            tqdm.write(f"Error: Individual variable {v} failed for {station_id} {year}-{month:02d}: {e}")
...
def fetch_era5_data(station, year, variables, max_workers=3, request_delay=1.0):
            ...
            try:
                if os.path.exists(c):
                    ...
                    if size >= GOOD_MIN_BYTES:
                        ...
                        tqdm.write(f"[{station_id}-{year}-{m:02d}] candidate {c} too small ({size} bytes) — will re-download")
            except Exception:
                continue
        return None
    ...
    def fetch_month(m):
        ...
        if cpath:
            tqdm.write(f"[{station_id}-{year}-{m:02d}] found existing candidate {cpath} — attempting local processing.")
            try:
                ...
                try:
                    if not (os.path.exists(per_file) and os.path.getsize(per_file) > 0):
                        ...
                        tqdm.write(f"[{station_id}-{year}-{m:02d}] saved per-month file {per_file} from candidate {cpath}")
                except Exception as e:
                    tqdm.write(f"[{station_id}-{year}-{m:02d}] failed saving per-month file from candidate: {e}")
                ...
                tqdm.write(f"[{station_id}-{year}-{m:02d}] processing existing candidate {cpath} failed: {e} — will try remote download/fallback.")
        ...
        for attempt in range(max_attempts):
            try:
                ...
                tqdm.write(f"[{station_id}-{year}-{m:02d}] submitting request (attempt {attempt + 1})")
                ...
                if tmpf.endswith(".zip") or (os.path.exists(tmpf) and not is_valid_nc(tmpf)):
                    try:
                        ...
                        tqdm.write(f"[{station_id}-{year}-{m:02d}] could not extract zip; continue to validation")
                ...
                try:
                    ds_point.to_netcdf(per_file)
                    tqdm.write(f"[{station_id}-{year}-{m:02d}] saved per-month file {per_file}")
                except Exception as e:
                    tqdm.write(f"[{station_id}-{year}-{m:02d}] failed to save per-month file: {e}")
                ...
                if attempt < max_attempts - 1:
                    tqdm.write(f"[{station_id}-{year}-{m:02d}] attempt {attempt + 1} failed: {e}, retrying...")
                    if os.path.exists(tmpf):
                        try:
                            os.remove(tmpf)
                        except Exception as e_remove:
                            tqdm.write(f"Could not remove {tmpf}: {e_remove}")
                    time.sleep(5)
                    continue
                else:
                    return (m, None, e)
        return (m, None, f"All download attempts failed for month {m}")
        ...
        if os.path.exists(per_file):
            try:
                ...
                if GOOD_MIN_BYTES <= size <= GOOD_MAX_BYTES:
                    tqdm.write(f"[{station_id}-{year}-{m:02d}] skipping submission — per-month file exists and is good ({per_file}, {size} bytes).")
                    results[m] = (None, None)
                    continue
                else:
                    tqdm.write(f"[{station_id}-{year}-{m:02d}] per-month file {per_file} exists but size {size} bytes not in good range -> will reprocess/download.")
            except Exception:
                tqdm.write(f"[{station_id}-{year}-{m:02d}] per-month file {per_file} exists but couldn't stat -> will reprocess/download.")
        if m in succeeded_months:
            tqdm.write(f"[{station_id}-{year}-{m:02d}] skipping submission — marked success in log.")
            results[m] = (None, None)
            continue
        months_to_submit.append(m)

    bar_desc = f"{Fore.CYAN}{Style.BRIGHT}[{station_id}-{year}]{Style.RESET_ALL}"
    pbar = tqdm(total=len(months_to_submit), desc=bar_desc, unit="month", dynamic_ncols=True, leave=True, file=sys.stdout)
        ...
        for fut in concurrent.futures.as_completed(future_to_month):
            ...
            try:
                ...
                if err is None:
                    ...
                    fails += 1
                pbar.set_postfix({"Success": f"{Fore.GREEN}{succ}{Style.RESET_ALL}",
                                  "Failed": f"{Fore.RED}{fails}{Style.RESET_ALL}",
                                  "Total": len(months_to_submit)})
            except Exception as e:
                tqdm.write(f"Unexpected thread error for {station_id} {year} month {m}: {e}")
                results[m] = (None, e)
                fails += 1
                pbar.set_postfix({"Success": f"{Fore.GREEN}{succ}{Style.RESET_ALL}",
                                  "Failed": f"{Fore.RED}{fails}{Style.RESET_ALL}",
    ...
    for m in months:
            ...
            try:
                ...
                if GOOD_MIN_BYTES <= size <= GOOD_MAX_BYTES:
                    try:
                        ...
                        tqdm.write(f"[{station_id}-{year}-{m:02d}] loaded per-month file {per_file} for assembly.")
                        continue
                    except Exception as e:
                        tqdm.write(f"Failed to open per-month file {per_file}: {e}")
                else:
                    tqdm.write(f"Per-month file {per_file} size {size} bytes not in good range -> will try candidates/fallback.")
            except Exception:
                tqdm.write(f"Couldn't stat per-month file {per_file} -> will try candidates/fallback.")
        if ds_point is not None:
            try:
                if not (os.path.exists(per_file) and os.path.getsize(per_file) > 0):
                    ds_point.to_netcdf(per_file)
                    tqdm.write(f"Saved per-month file (from in-memory ds) {per_file}")
            except Exception as e:
                tqdm.write(f"Unable to save per-month file: {e}")
        ...
        for cand in candidates:
            if os.path.exists(cand):
                ...
                if size >= GOOD_MIN_BYTES:
                    try:
                        tqdm.write(f"Processing existing candidate {cand} for assembly.")
                        ds_tmp = process_downloaded_file(cand, lat, lon)
                        try:
                            ds_tmp.to_netcdf(per_file)
                            tqdm.write(f"Saved per-month file {per_file} from candidate {cand}")
                        except Exception as e:
                            tqdm.write(f"Couldn't save per-month file after processing candidate: {e}")
                        ...
                        tqdm.write(f"Failed processing candidate {cand}: {e}")
                        continue
                else:
                    tqdm.write(f"Candidate {cand} size {size} bytes is too small — skipping")
        if found_and_processed:
            continue

        tqdm.write(f"Invoking per-variable fallback")
        ...
        if fallback is not None:
            try:
                ...
                tqdm.write(f"Saved per-month file (fallback) {per_file}")
            except Exception as e:
                tqdm.write(f"Failed to save fallback per-month file: {e}")
            log_download_status(station_id, year, m, list(variables.values()), "success")
            all_month_ds.append(fallback)
            tqdm.write(f"Per-variable fallback succeeded for {station_id} {year}-{m:02d}")
        else:
            tqdm.write(f"All attempts failed for {station_id} {year}-{m:02d}")
...
def main():
    tqdm.write(f"\n{Fore.CYAN}{Style.BRIGHT}=== ERA5 Data Fetching Tool ==={Style.RESET_ALL}\n")
    ...
    if SELECT_STATIONS:
        ...
        if stations.empty:
            tqdm.write(f"No stations matched SELECT_STATIONS={SELECT_STATIONS}")
            return
    if SELECT_YEARS:
    ...
    if TEST_STATION:
        try:
            ...
            tqdm.write("TEST_STATION format must be STATION:YEAR (e.g. DL009:2021)")
            return
        stations = stations[stations['file_name'] == st_name]
        if stations.empty:
            tqdm.write(f"No station '{st_name}' found in config/stations.csv")
            return
        years = [st_year]
        TEST_MODE = True
        tqdm.write(f"TEST_STATION enabled: {st_name} {st_year}")

    Path(RAW_DATA_DIR).mkdir(parents=True, exist_ok=True)
    Path('data/processed/era5').mkdir(parents=True, exist_ok=True)
    tqdm.write(f"RAW_DATA_DIR is: {RAW_DATA_DIR}")

    total_stations = len(stations)
    total_years = len(years)
    tqdm.write(f"Starting processing for {total_stations} station(s) and {total_years} year(s)\n")

    master_total = total_stations * total_years
    master_bar = tqdm(total=master_total, desc=f"{Fore.MAGENTA}Overall Progress{Style.RESET_ALL}", unit="station-year", dynamic_ncols=True, leave=True, file=sys.stdout)

    for i, (_, st) in enumerate(stations.iterrows(), 1):
        for j, y in enumerate(years, 1):
            ...
            tqdm.write(f"\nProcessing station {i}/{total_stations}: {st.file_name}, year {j}/{total_years}: {y}")
            if SKIP_EXISTING and os.path.exists(nc_out) and os.path.exists(csv_out):
                try:
                    ...
                    if GOOD_MIN_BYTES <= size_nc <= GOOD_MAX_BYTES:
                        tqdm.write(f"Skipping {st.file_name} {y} — already downloaded and processed (annual NC size {size_nc} bytes).")
                        master_bar.update(1)
                        continue
                    else:
                        tqdm.write(f"Not skipping {st.file_name} {y}: annual NC {nc_out} size {size_nc} bytes not in good range -> will reprocess/update.")
                except Exception:
                    tqdm.write(f"Not skipping {st.file_name} {y}: couldn't stat {nc_out} -> will reprocess/update.")
            try:
                tqdm.write(f"Starting fetch for station {st.file_name} year {y}")
                ds = fetch_era5_data(st, y, variables, max_workers=MAX_WORKERS, request_delay=REQUEST_DELAY)
                if ds is None:
                    tqdm.write(f"No data for {st.file_name} {y}")
                    master_bar.update(1)
                    continue
                ds.to_netcdf(nc_out)
                tqdm.write(f"Saved NetCDF: {nc_out}")
                df = ds.to_dataframe().reset_index()
                df = add_derived_features_df(df)
                df.to_csv(csv_out, index=False)
                tqdm.write(f"Saved CSV: {csv_out}")
                tqdm.write(f"Successfully processed {st.file_name} {y}")
            except Exception as e:
                tqdm.write(f"Top-level error for {st.file_name} {y}: {e}")
                log_download_status(st.file_name, y, "all", list(variables.values()), "failed", str(e))
            master_bar.set_postfix({"Station": f"{i}/{total_stations}", "Year": f"{y}"})
            master_bar.update(1)
            master_bar.refresh()
            sys.stdout.flush()
